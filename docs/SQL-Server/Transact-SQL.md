...
Transact-SQL（通常缩写为 T-SQL）是微软和Sybase公司共同开发的一种关系数据库通用语言SQL的扩展。它在标准SQL的基础上增加了编程功能，如变量、流程控制、函数等，主要用于Microsoft SQL Server和Sybase Adaptive Server等数据库管理系统中。
...
7. **存储过程与函数**：
   - 存储过程（Stored Procedure）：预编译的SQL代码块，可带参数，提高性能和复用性。
   - 用户定义函数（UDF）：可返回标量值或表。
...
下面给你 3 条“能直接下到手、用 SSDT 打开就能跑”的**开源 Transact-SQL 演练项目**，全部按“先易后难”排好，**5 分钟就能在 Visual Studio 里按 F5 调试**，最适合快速把 T-SQL + SSDT 的完整工作流跑通。  
...
1 分钟拿到 WideWorldImporters-SSDT 源码
打开官方仓库
https://github.com/microsoft/sql-server-samples
点右侧 Releases → 找到 “WideWorldImporters SSDT sample database”（标签名 wwi-ssdt）→ 下载 Source code (zip)。
解压后路径
samples\databases\wide-world-importers\wide-world-importers-ssdt\WideWorldImporters.sqlproj
...
仓库：httpsgithub.com/tSQLt-org/tSQLt-SSDT-Template  
- 把主流单元测试框架 tSQLt 直接打包成 SSDT 项目，**F5 后自动部署 tSQLt 与示例测试**。  
...
1. 装 SSDT  
   打开 Visual Studio Installer → 勾选“SQL Server Data Tools”工作负载。  

2. 拿代码  
   git clone httpsgithub.com/microsoft/sql-server-samples/releases/tag/wwi-ssdt  
   双击 WideWorldImporters.sqlproj 打开。  

3. 配调试库  
   右键项目 → 属性 → 调试 → 新建 LocalDB 或指向自己的 SQL Server 实例。  
...
4. F5 一键部署+调试  
   ...
   - 自动 publish 到调试库  
   - 附加调试器  
   随后即可在存储过程里打断点、单步、看变量。  

------------------------------------------------
学完这三套示例，你就把  
“项目结构→T-SQL 语法→调试→单元测试→Git 版本控制→CI/CD 发布”  
整条链路全跑通了，再去看自己公司的业务库就能直接上手。祝演练愉快!

| 对比维度      | Northwind-SSDT（经典版）                      | WideWorldImporters-SSDT（现代版）                                                                            |
| ------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| 设计年代      | 1996 年随 Office 发布，专为 90 年代小商户场景 | 2016 年随 SQL Server 2016 重新设计，面向云端与现代 ERP                                                       |
| 目标 SQL 版本 | SQL Server 2000-2012 语法为主                 | SQL Server 2016+ / Azure SQL DB，必须用 2016 SP1 以上才能跑全功能                                            |
| 项目体积      | ≈ 1-2 MB，几十张表，数据 1-10 MB 级           | 空库 30 MB，一键生成 3.5 年数据后 ≈ 160 MB（可再膨胀到 GB 级）                                               |
| 新特性覆盖    | 无（没有列存、内存、JSON、时态表、分区）      | 全部都有：列存、内存 OLTP、时态表、JSON、分区、数据掩码、行级安全等                                          |
...
“多久能掌握”完全取决于你现在的 SQL 起点和每天能投入的时间。下面给你 3 个常见起点 + 对应路线图，**按“工作日每天 1.5 h”估算**，你可以直接对号入座。所有任务都围绕 **Northwind-SSDT → WideWorldImporters-SSDT → 自己开新项目** 这条主线展开。
...
| 阶段         | 目标                  | 任务清单（可量化）                                                                             | 耗时                      |
| ------------ | --------------------- | ---------------------------------------------------------------------------------------------- | ------------------------- |
| ① 入门       | 把 SSDT 玩顺          | 把 Northwind-SSDT 克隆→F5 成功→单步调试 5 个存储过程                                           | 3 天                      |
| ② 基础 SQL   | 写 CRUD + 视图 + 事务 | 在 Northwind 里自己写 10 个查询、3 个视图、2 个带事务的存储过程并通过 tSQLt 测试               | 7 天                      |
| ③ 迁移现代库 | 体验“大数据量”        | 把 Northwind 的订单表导入 WideWorldImporters，跑通 `PopulateDataToCurrentDate` 生成 100 万订单 | 3 天                      |
...
4. 周末集中 2 h 做“综合演练”——写个迷你业务需求（例如“给 Northwind 加会员积分表”）从需求→建表→存储过程→测试→Git 提交一条龙，**一次性把知识缝起来**。
